...
// Adapted from React Bits' optimized PixelSwap. See LICENSE.md.
import {
  CSSProperties,
  KeyboardEvent,
  ReactNode,
...
// Bound the animated mask's complexity regardless of the requested pixel size.
...
const buildGrid = ({
  ...
  while (columns * rows > MAX_PIXELS) {
    size = Math.max(size + 1, Math.ceil(size * Math.sqrt((columns * rows) / MAX_PIXELS)));
...
// Animate only the mask windows. The single incoming snapshot stays fixed.
...
function PixelSwap({
  ...
  const snapshotRef = useRef<SVGForeignObjectElement | null>(null);
  const maskId = `pixel-swap-${useId()}`;
  const animationsRef = useRef<Animation[]>([]);
  const completionRef = useRef<{ active: boolean; notify?: (active: boolean) => void } | null>(null);
  ...
  const config = { duration, pixelDuration, pixelSpin, pixelScale, pixelRadius, fade, easing, onComplete, onPrepare };
  ...
  useLayoutEffect(() => {
    ...
    if (transition || !completion || completion.active !== shownActive) return;
    completionRef.current = null;
    // Resume live content and focus only after its layer has been committed.
    completion.notify?.(shownActive);
  }, [shownActive, transition]);
  ...
  useEffect(() => {
    ...
    // Synchronize the controlled request with the imperative animation lifecycle.
  ...
  useEffect(() => {
    ...
    const finish = () => {
      if (cancelled) return;
      cancelled = true;
      // Keep the finished mask and snapshot visible until React commits the
      // incoming layer. The effect cleanup then releases the old overlay.
      completionRef.current = { active: to, notify: settings.onComplete };
      setShownActive(to);
      setTransition(null);
    };
    ...
    const animate = async () => {
      // MapLibre redraws synchronously here, before its WebGL buffer is cleared.
      // Capture once for the shared masked layer, without mounting another map.
      settings.onPrepare?.(to);
      ...
      const copies = snapshot.querySelectorAll('canvas');
      canvases.forEach((canvas, index) => {
        // There is now only one snapshot, so its canvas can retain the pixels
        // directly. Copy before WebGL clears the source buffer, without a PNG
        // encode/decode round trip or one full canvas per mask window.
        const copy = copies[index];
        copy.width = canvas.width;
        copy.height = canvas.height;
        const context = copy.getContext('2d');
        if (!context) throw new Error('Canvas snapshot unavailable');
        context.drawImage(canvas, 0, 0);
      });
      if (cancelled) return;

      snapshot.style.width = `${frozenGrid.width}px`;
      snapshot.style.height = `${frozenGrid.height}px`;
      ...
      // Wall-clock timers can fire before the browser presents the last frame,
      // especially during scrolling or after a background tab resumes.
      await Promise.all(animationsRef.current.map(animation => animation.finished));
      finish();
    };
    // If a canvas cannot be captured, reveal the live layer directly.
    void animate().catch(finish);
    return () => {
      cancelled = true;
      stopAnimations();
    };
  ...
  return (
    ...
    >
      ...
      {transition && (
        <svg className="pixel-swap__grid" width={transition.grid.width} height={transition.grid.height} aria-hidden="true">
          <defs>
            <mask id={maskId} maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" x="0" y="0" width={transition.grid.width} height={transition.grid.height} style={{ maskType: 'alpha' }}>
              {transition.grid.pixels.map((pixel, index) => (
                <rect
                  ...
                  rx={transition.grid.size * clamp(pixelRadius, 0, 50) / 100}
                />
              ))}
            </mask>
          </defs>
          <foreignObject ref={snapshotRef} width={transition.grid.width} height={transition.grid.height} mask={`url(#${maskId})`} />
        </svg>
      )}
    </div>
  );
